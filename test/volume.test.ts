...
describe('VolumeService', () => {
  ...
  const mockConfig = {
    networks: {
      ethereum: {
        ...
        quoter: new ethers.Contract('0x0000000000000000000000000000000000000000', [], new ethers.providers.JsonRpcProvider('http://localhost:8545')),
        tokens: {
          WETH: new Token(1, '0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2', 18, 'WETH', 'Wrapped Ether'),
          SEED: new Token(1, '0x5eed99d066a8CaF10f3E4327c1b3D8b673485eED', 18, 'SEED', 'Seed Token')
        }
      },
      arbitrum: {
        chainId: 42161,
        ...
        quoter: new ethers.Contract('0x0000000000000000000000000000000000000000', [], new ethers.providers.JsonRpcProvider('http://localhost:8546')),
        tokens: {
          WETH: new Token(42161, '0x82aF49447D8a07e3bd95BD0d56f35241523fBab1', 18, 'WETH', 'Wrapped Ether'),
          SEED: new Token(42161, '0x86f65121804D2Cdbef79F9f072D4e0c2eEbABC08', 18, 'SEED', 'Seed Token')
        }
      }
    },
    tokens: {
  ...
  beforeEach(() => {
    ...
    const mockNetworks = new Map(Object.entries(mockConfig.networks)) as Map<string, NetworkConfig>;
    ...
    mockTradeService = new TradeService(mockNetworks) as jest.Mocked<TradeService>;
    mockCoinGeckoService = new CoinGeckoService() as jest.Mocked<CoinGeckoService>;
  ...
  describe('Volume Tracking', () => {
    it('should initialize volume tracking for all networks', () => {
      ...
      mockNetworkService.getNetworks.mockReturnValue(new Map(networks.map(n => [n, mockConfig.networks[n]])));
    ...
    it('should track volume correctly', async () => {
      ...
      // Mock volume data
      (mockPoolService as any).getPoolVolume = jest.fn().mockResolvedValue(mockVolume);

      await volumeService.manualVolumeCheck();
      const status = volumeService.getVolumeStatus() as VolumeStatus;
      expect(status.networkVolumes).toContainEqual([mockNetworkKey, mockVolume]);
    });
  });
  ...
  describe('Volume Rebalancing', () => {
    it('should trigger rebalancing when volume is below target', async () => {
      ...
      mockTradeService.executeRebalanceTrades.mockResolvedValue(mockRebalanceResult);
    ...
    it('should handle rebalancing failures gracefully', async () => {
      ...
      await volumeService.manualVolumeCheck();
      const status = volumeService.getVolumeStatus() as VolumeStatus;
      expect(status.rebalanceInProgress).toContainEqual([mockNetworkKey, false]);
    });
  });
  ...
  describe('Error Handling', () => {
    it('should handle network errors during volume check', async () => {
      (mockPoolService as any).getPoolVolume = jest.fn().mockRejectedValue(new Error('Network error'));

      await expect(volumeService.manualVolumeCheck()).rejects.toThrow('Network error');
    });
    ...
    it('should handle price fetch errors during rebalancing', async () => {
      ...
      mockCoinGeckoService.fetchPrices.mockRejectedValue(new Error('Price fetch failed'));
