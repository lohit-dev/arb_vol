...
import { ExternalAPIVolumeTracker } from "./src/services/dexVolume";
...
class RobustArbitrageBot {
  ...
  private async initializeNonceManagers(): Promise<void> {
    for (const [networkKey, network] of this.networks.entries()) {
      try {
        ...
        // Get the latest nonce from the network (both pending and confirmed)
        const currentNonce = await network.provider.getTransactionCount(
        ...
        // Fallback to 0 if initialization fails
        this.nonceManagers.set(networkKey, 0);
      }
    }
  }

  private getNextNonce(networkKey: string): number {
    const currentNonce = this.nonceManagers.get(networkKey) || 0;
    this.nonceManagers.set(networkKey, currentNonce + 1);
    return currentNonce;
  }

  private resetNonce(networkKey: string): void {
    const currentNonce = this.nonceManagers.get(networkKey) || 0;
    if (currentNonce > 0) {
      this.nonceManagers.set(networkKey, currentNonce - 1);
    }
  ...
  private async fetchNetworkVolume(networkKey: string): Promise<number> {
    try {
      ...
      // Get volume from DEX Screener API
      const externalVolume =
        await this.externalVolumeTracker.getDexScreenerVolume(
          poolAddress,
          chainId
        );
      ...
      // Fallback to on-chain method if API fails
      console.log(
        `⚠️ DEX Screener API failed, falling back to on-chain volume calculation`
      );

      // Your existing on-chain volume calculation code here...
      ...
      for (const event of events) {
        ...
        volume += amount1 * prices.ethereum.usd;
      }
  ...
  private async executeTrade(params: TradeParams): Promise<{
    ...
    try {
      ...
      // Execute swap with nonce
      const nonce = this.getNextNonce(params.network.toLowerCase());
      console.log(`   Executing swap with nonce ${nonce}`);
      ...
      this.resetNonce(params.network.toLowerCase());

      // Simplified error message
      const errorMessage = error.reason || error.message;
      const simpleError = errorMessage.split("(")[0].trim(); // Get just the main error message

      console.error(`Trade failed: ${simpleError}`);
...
async function main(): Promise<void> {
  ...
  // FIX THIS LINE - properly await the volume check:
  await bot.manualVolumeCheck(); // Remove .catch(console.error) and use await
